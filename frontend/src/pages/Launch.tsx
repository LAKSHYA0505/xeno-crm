  ...
  interface Stats {
    ...
    opened:       number
    clicked:      number
    converted:    number        // ← new
    totalRevenue: number | null // ← new
    failed:       number
  ...
  export default function Launch() {
    ...
    async function handleBuild() {
      ...
      try {
        ...
        setPreview({ count, sample, rules, message: message ?? '' })
    ...
    return (
      <div className="max-w-2xl mx-auto space-y-6">
            ...
            <Card>
              ...
              <CardContent className="space-y-3">
                <div className="flex items-baseline gap-1.5">
                  <span className="text-4xl font-bold text-indigo-600">{preview.count}</span>
                  <span className="text-sm text-muted-foreground">customers matched</span>
                </div>
        ...
        {/* ══ STAGE 4 — LIVE STATS ══ */}
        {stage === 'stats' && stats && (() => {
          const accounted = stats.delivered + stats.failed
          return (
            <div className="space-y-4">
              {/* Success banner */}
              <div
              ...
              {/* Live stat tiles — now 6 columns including Orders */}
              <Card>
                <CardContent className="py-6">
                  <div className="grid grid-cols-6 gap-4">
                    ...
                    <StatTile
                      ...
                      sub={`₹${stats.totalRevenue?.toLocaleString() ?? "0"}`}
                      color="text-emerald-600"
                    />
                  </div>

                  <p className="text-xs text-muted-foreground mt-4">
                    Delivered includes opened, clicked &amp; converted.{" "}
                  ...
                  {/* Live funnel bar — includes Orders + Failed */}
                      ...
                      >
                        ...
                        <div className="flex-1 bg-muted rounded-full h-1.5 overflow-hidden">
                          <div
                            ...
                            style={{
                              width: `${pctWidth(value, stats.totalLogs)}%`,
                            }}
                          />
                        </div>
              ...
              {/* Revenue banner — shown only when conversions exist */}
              {stats.converted > 0 && (
                <div
                  className="bg-emerald-50 dark:bg-emerald-950/30 border border-emerald-200
                                dark:border-emerald-800 rounded-lg px-4 py-3 flex items-center justify-between"
                >
                  <span className="text-sm font-medium text-emerald-700 dark:text-emerald-400">
                    💰 {stats.converted} orders driven by this campaign
                  </span>
              ...
              <Card>
                ...
                <CardContent>
                  {loadingSummary && (
                    <AiThinking label="Analyzing campaign performance…" />
                  )}
